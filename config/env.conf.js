/*
 Node production environmental variables
...
/*
 Check each necessary node `environment variable` to see if a value has been set and if not, use the `config`
 object to supply appropriate values
 */
...
function validateNodeEnvironment() {
    /*
     Check to see that the `process.env.NODE_ENV has been set to an appropriate value of `development`, `production`
...
// Set the appropriate MongoDB URI with the `config` object based on the value in `process.env.NODE_ENV
